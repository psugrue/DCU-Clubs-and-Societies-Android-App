...
if (!empty($_POST)) {
    ...
    if (empty($_POST['username']) || empty($_POST['password']) || empty($_POST['email'])) {
        ...
        $response["success"] = 0;
        $response["message"] = "Please Enter Email,Username and Password.";
    ...
    if ($row) {
		...
		if($row){
				$response["success"] = 0;
				$response["message"] = "I'm sorry, that email is already in use";
				die(json_encode($response));
		}
    ...
    $query = "INSERT INTO users ( username, encrypt_password, email ) VALUES ( :user, :pass, :email) ";
		
		$password = $_POST['password'];
		$hash = hashEncrypt($password);
